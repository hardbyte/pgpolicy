//! Testable CLI logic for pgpolicy.
//!
...
use pgpolicy_core::diff::{self, Change};
use pgpolicy_core::manifest::{self, ExpandedManifest, PolicyManifest};
use pgpolicy_core::model::RoleGraph;
use pgpolicy_core::sql;
...
"#;
    ...
    fn read_nonexistent_file_fails() {
        let result = read_manifest_file(Path::new("/tmp/nonexistent-pgpolicy-test.yaml"));
        assert!(result.is_err());
