//! pgpolicy CLI — declarative PostgreSQL role policy manager.
...
use pgpolicy_inspect::InspectConfig;

// ---------------------------------------------------------------------------
// CLI argument definitions
// ---------------------------------------------------------------------------

/// pgpolicy — declarative PostgreSQL role & privilege manager.
///
/// Define roles, grants, default privileges, and memberships in a YAML manifest
/// and converge your database to match.
#[derive(Parser)]
#[command(name = "pgpolicy", version, about, long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Validate {
        ...
        #[arg(short, long, default_value = "pgpolicy.yaml")]
        file: PathBuf,
    },
...
async fn inspect_current(
    pool: &PgPool,
    validated: &pgpolicy_cli::ValidatedManifest,
) -> Result<pgpolicy_core::model::RoleGraph> {
    ...
    pgpolicy_inspect::inspect(pool, &config)
        .await
