...
const FINALIZER: &str = "pgpolicy.io/finalizer";
...
pub enum ReconcileError {
    ...
    ManifestExpansion(#[from] pgpolicy_core::manifest::ManifestError),

    #[error("database inspection error: {0}")]
    Inspect(#[from] pgpolicy_inspect::InspectError),
...
async fn reconcile_apply_inner(
    ...
    let expanded = pgpolicy_core::manifest::expand_manifest(&manifest)?;
    ...
    let has_database_grants = expanded
        .grants
        .iter()
        .any(|g| g.on.object_type == pgpolicy_core::manifest::ObjectType::Database);
    let inspect_config =
        pgpolicy_inspect::InspectConfig::from_expanded(&expanded, has_database_grants);
    let current = pgpolicy_inspect::inspect(&pool, &inspect_config).await?;

    // 6. Compute diff.
    let changes = pgpolicy_core::diff::diff(&current, &desired);
...
fn accumulate_summary(summary: &mut ChangeSummary, change: &pgpolicy_core::diff::Change) {
    use pgpolicy_core::diff::Change;
    match change {
    ...
    F: FnOnce(&mut PostgresPolicyStatus),
{
    ...
    api.patch_status(
        &name,
        &PatchParams::apply("pgpolicy-operator"),
        &Patch::Merge(&patch),
    )
    .await?;
...
mod tests {
    ...
    fn accumulate_summary_counts() {
        use pgpolicy_core::diff::Change;
        use pgpolicy_core::model::RoleState;
        ...
        accumulate_summary(
            ...
            &Change::Grant {
                role: "test".to_string(),
                object_type: pgpolicy_core::manifest::ObjectType::Schema,
                schema: None,
                name: Some("public".to_string()),
                privileges: [pgpolicy_core::manifest::Privilege::Usage]
                    .into_iter()
                    .collect(),
            },
        );
