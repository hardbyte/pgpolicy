...
//!
//! Defines the `pgpolicy.io/v1alpha1` CRD that the operator watches.
...
///
/// This mirrors `pgpolicy_core::manifest::Profile` but derives `JsonSchema`.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
...
impl PostgresPolicySpec {
    ...
    pub fn to_policy_manifest(&self) -> pgpolicy_core::manifest::PolicyManifest {
        use pgpolicy_core::manifest::{
        ...
        let memberships = self
            .memberships
            .iter()
            .map(|m| pgpolicy_core::manifest::Membership {
                role: m.role.clone(),
                members: m
                    .members
                    .iter()
...
mod tests {
    ...
    fn crd_generates_valid_schema() {
        ...
        assert!(yaml.contains("pgpolicy.io"), "group should be pgpolicy.io");
