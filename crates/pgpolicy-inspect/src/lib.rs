//! Database introspection for pgpolicy.
//!
//! Queries `pg_catalog` tables to build a [`pgpolicy_core::model::RoleGraph`]
...
use pgpolicy_core::model::RoleGraph;
...
pub struct InspectConfig {
    /// The role names that the manifest manages (created by pgpolicy).
...
impl InspectConfig {
    ...
    pub fn from_expanded(
        expanded: &pgpolicy_core::manifest::ExpandedManifest,
        ...
        for grant in &expanded.grants {
            ...
            if grant.on.object_type == pgpolicy_core::manifest::ObjectType::Schema
...
mod tests {
    use super::*;
    use pgpolicy_core::manifest::{expand_manifest, parse_manifest};
